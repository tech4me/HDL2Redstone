...
#define CHECKUPDATEGRAPH(x, y, z, TempP) \
    ...
    for (int i = 0; i < Connections.size(); i++) {
        ...
        if (!FailedWireSingleRouting) {
            ...
            if (!regularRoute(*FailedWireSingleRoutingTemp)) {
                ...
                if (next_times > REPEATER_FIXING_TIMES || (!next_pass_flag)) {
                    std::cout << "Repeater self loop cannot be fixed: " << FailedWireSingleRoutingTemp->getName()
                              << std::endl;
...
bool Router::reRouteNextIllegal(
    Connection& C,
    ...
    for (auto& it : next_congest) {
        ...
        if (getUsedSpace(std::get<0>(it.first), std::get<1>(it.first), std::get<2>(it.first)) != 1) {
            getUsedSpace(std::get<0>(it.first), std::get<1>(it.first), std::get<2>(it.first)) = 3;
        }
    }
    std::for_each(Points.begin(), Points.end(), [this](Point& P) {
        P.cost = D.Width * D.Height * D.Length;
    ...
    for (auto& it : next_congest) {
        if (getUsedSpace(std::get<0>(it.first), std::get<1>(it.first), std::get<2>(it.first)) == 3) {
            ...
            getUsedSpace(std::get<0>(it.first), std::get<1>(it.first), std::get<2>(it.first)) = 0;
        }
    }
    if (ReRouteFlag) {
        FailedWireSingleRouting = NULL;
        DOUT(<< "ReRouting for NextIllegal Success: " << C.getName() << std::endl);
        return true;
    } else {
        for (auto& it : next_congest) {
            // TODO what if usedspace is 2?
            // if(non >= 15){
            // std::cout<<"PAIR1: "<<std::get<0>(it.first)<<" "<<std::get<1>(it.first)<<" "<<std::get<2>(it.first)<<"
            // used "<<getUsedSpace(std::get<0>(it.first), std::get<1>(it.first), std::get<2>(it.first))<<std::endl;
            // std::cout<<"PAIR2: "<<std::get<0>(it.second)<<" "<<std::get<1>(it.second)<<" "<<std::get<2>(it.second)<<"
            // used "<<getUsedSpace(std::get<0>(it.second), std::get<1>(it.second), std::get<2>(it.second))<<std::endl;
            //}
            ...
            // if(non >= 15){
            // std::cout<<"AFTER PAIR1: "<<std::get<0>(it.first)<<" "<<std::get<1>(it.first)<<"
            // "<<std::get<2>(it.first)<<" used "<<getUsedSpace(std::get<0>(it.first), std::get<1>(it.first),
            // std::get<2>(it.first))<<std::endl; std::cout<<"AFTER PAIR2: "<<std::get<0>(it.second)<<"
            // "<<std::get<1>(it.second)<<" "<<std::get<2>(it.second)<<" used "<<getUsedSpace(std::get<0>(it.second),
            // std::get<1>(it.second), std::get<2>(it.second))<<std::endl;
            //}
        }
        std::for_each(Points.begin(), Points.end(), [this](Point& P) {
            P.cost = D.Width * D.Height * D.Length;
        ...
        for (auto& it : next_congest) {
            if (getUsedSpace(std::get<0>(it.second), std::get<1>(it.second), std::get<2>(it.second)) == 3) {
                // TODO what if original usedspace is 2?
                getUsedSpace(std::get<0>(it.second), std::get<1>(it.second), std::get<2>(it.second)) = 0;
            }
        }
        if (ReRouteFlagInner) {
            FailedWireSingleRouting = NULL;
...
bool Router::regularRoute(Connection& C) {
    ...
    auto cmp = [this, &endTemp](const Point* lhs, const Point* rhs){return (lhs->cost + this->heuristic(endTemp, lhs)) > (rhs->cost + this->heuristic(endTemp, rhs));};
    std::priority_queue<Router::Point*, std::vector<Router::Point*>, decltype(cmp)> Q(cmp);
    ...
    while (!Q.empty()) {
        ...
        Q.pop();
        // if(TempP->Loc.x==11 && TempP->Loc.y==1 && TempP->Loc.z==8){
        //     std::cout<<"hi reach here"<<std::endl;
        // }        
        TempP->visited = 1;
        ...
        if (TempP->Loc.x /*&& (TempP->length <= MAX_NUM_OF_WIRE || TempP->ori == HDL2Redstone::Orientation::OneCW)*/ &&
            (TempP->Loc.y) % 2 == 1) {
            CHECKUPDATEGRAPH(TempP->Loc.x - 1, TempP->Loc.y, TempP->Loc.z, TempP);
        }
        ...
        if (TempP->Loc.z /*&& (TempP->length <= MAX_NUM_OF_WIRE || TempP->ori == HDL2Redstone::Orientation::TwoCW)*/ &&
            (TempP->Loc.y) % 2 == 1) {
            CHECKUPDATEGRAPH(TempP->Loc.x, TempP->Loc.y, TempP->Loc.z - 1, TempP);
        }
        ...
        if (TempP->Loc.x && (TempP->Loc.y > 1) /*&& (TempP->length <= MAX_NUM_OF_WIRE)*/) {
            CHECKUPDATEGRAPH(TempP->Loc.x - 1, TempP->Loc.y - 1, TempP->Loc.z, TempP);
        }
        ...
        if (TempP->Loc.z && (TempP->Loc.y > 1) /* && (TempP->length <= MAX_NUM_OF_WIRE)*/) {
            CHECKUPDATEGRAPH(TempP->Loc.x, TempP->Loc.y - 1, TempP->Loc.z - 1, TempP);
        }
    ...
    for (auto it : end) {
        ...
        if (getPoint(it.x, it.y, it.z).P) {
            ...
            C.setInsert(Connection::ConnectionResult(std::make_tuple(TempX, TempY - 1, TempZ),
                                                     D.CellLib.getCellPtr("WIRE"), getPoint(it.x, it.y, it.z).ori, -1,
                                                     PartID));
...
bool Router::reRouteIllegal(
    ...
    for (auto& i : congestionPoints) {
        ...
        for (auto& it : congestionPoints_prev) {
            ...
            if ((std::get<0>(it) == std::get<0>(i)) && (std::get<1>(it) == std::get<1>(i) + 1) &&
                (std::get<2>(it) == std::get<2>(i))) {
                if (std::get<3>(it) == 0) {
                    temp_flag = true;
                    DOUT(<< "        get it twice: disable the lower point for illegal" << std::endl);
                    if (getUsedSpace(std::get<0>(it), std::get<1>(it), std::get<2>(it)) == 3) {
                        getUsedSpace(std::get<0>(it), std::get<1>(it), std::get<2>(it)) = 0;
                        DOUT(<< "        get it twice: recover it: " << std::get<0>(it) << " " << std::get<1>(it) << " "
                             << std::get<2>(it) << std::endl);
                    }
                    congestionPoints_prev.erase(it);
                    congestionPoints_prev.insert(std::make_tuple(std::get<0>(i), std::get<1>(i) - (std::get<3>(i) == 0),
                                                                 std::get<2>(i), 1, std::get<3>(i)));
                    inserted_congestion_points.insert(
                        std::make_tuple(std::get<0>(i), std::get<1>(i) - (std::get<3>(i) == 0), std::get<2>(i)));
        ...
        if (!temp_flag) {
            inserted_congestion_points.insert(std::make_tuple(std::get<0>(i), std::get<1>(i) + 1, std::get<2>(i)));
            local_congestion_points.insert(
                std::make_tuple(std::get<0>(i), std::get<1>(i) + 1, std::get<2>(i), 0, std::get<3>(i)));
        }
    }
    for (auto it : congestionPoints_prev) {
    ...
    //     Usedspace "<<getUsedSpace(std::get<0>(*it),std::get<1>(*it),std::get<2>(*it))<<std::endl;
    // }
    // }
...
bool Router::reRouteIllegalHelper(
    ...
    do {
        for (auto it = local_congestion_points.begin(); it != local_congestion_points.end(); ++it) {
            // TODO what if usedspace is 2?
            // std::cout<<"to set to 3: "<<std::get<0>(*it)<<" "<<std::get<1>(*it)<<" "<<std::get<2>(*it)<<" used
            // "<<getUsedSpace(std::get<0>(*it), std::get<1>(*it), std::get<2>(*it))<<std::endl;
                ...
                for (auto mIterator = RetIllegalPoints.begin(); mIterator != RetIllegalPoints.end(); mIterator++) {
                    if ((std::get<0>(*nIterator) == std::get<0>(*mIterator)) &&
                        ...
                        local_congestion_points_temp.insert(std::make_tuple(
                            std::get<0>(*nIterator), std::get<1>(*nIterator) - (std::get<4>(*nIterator) == 0) - 1,
...
bool Router::reRouteStartRouting(coord congestionPoint) {
    ...
    for (auto& it : CPtrs) {
        ...
        DOUT(<< it->getName() << " is rolling back for ReRouting" << std::endl);
        it->Result.clear();
...
bool Router::checkUpdateGraphHelper(Router::Point* Parent, Router::Point* Current) {
    ...
    if(Current->length == MAX_NUM_OF_WIRE + 1 && (Current->ori != ori_ || Parent->Loc.y != Current->Loc.y)){
        //std::cout<<"reach here "<<Current->Loc.x<<" "<<Current->Loc.y<<" "<<Current->Loc.z<<std::endl;
        RetFlag = false;
    }else
    {
    ...
    if (RetFlag) {
        ...
        // if(Parent->Loc.x==11 && Parent->Loc.y==1 && Parent->Loc.z==8){
        //     std::cout<<"reach here "<<RetFlag<<" "<<Current->Loc.x<<" "<<Current->Loc.y<<" "<<Current->Loc.z<<std::endl;
        // }
        // if(Current->Loc.x==11 && Current->Loc.y==1 && Current->Loc.z==8){
        //     std::cout<<"okokokreach here "<<RetFlag<<" "<<Parent->Loc.x<<" "<<Parent->Loc.y<<" "<<Parent->Loc.z<<std::endl;
        // }
    return RetFlag;
}
...
Router::coord Router::updateSinglePortUsedSpace(std::tuple<uint16_t, uint16_t, uint16_t> Loc, Facing Fac,
    ...
    if (Fac == Facing::North) {
        ...
        if (getUsedSpace(Coord.X, Coord.Y - 1, Coord.Z - 1) == 1 ||
            getUsedSpace(Coord.X, Coord.Y - 1, Coord.Z - 1) == 3) {
            congestion.y--;
        }
    } else if (Fac == Facing::East) {
        ...
        if (getUsedSpace(Coord.X + 1, Coord.Y - 1, Coord.Z) == 1 ||
            getUsedSpace(Coord.X + 1, Coord.Y - 1, Coord.Z) == 3) {
            congestion.y--;
        }
    } else if (Fac == Facing::South) {
    ...
    for (auto& n : T.HoldViolatedPaths) {
        ...
        for (auto& m : n.CombPath) {
            ...
            if (index < SinkPortConnections.size()) { // find retiming partial path
                ...
                //     std::cout<<"point list "<<std::get<0>(test)<<" "<<std::get<1>(test)<<"
                //     "<<std::get<2>(test)<<std::endl;
                // }
                for (int SR_index = m.first->SubResult[index].size() - 2; SR_index > 0; SR_index--) {
                    if (((std::get<0>(m.first->SubResult[index][SR_index + 1]) -
                          std::get<0>(m.first->SubResult[index][SR_index])) ==
                         (std::get<0>(m.first->SubResult[index][SR_index]) -
                          std::get<0>(m.first->SubResult[index][SR_index - 1]))) &&

                        ((std::get<2>(m.first->SubResult[index][SR_index + 1]) -
                          std::get<2>(m.first->SubResult[index][SR_index])) ==
                         (std::get<2>(m.first->SubResult[index][SR_index]) -
                          std::get<2>(m.first->SubResult[index][SR_index - 1]))) &&

                        (std::get<1>(m.first->SubResult[index][SR_index + 1]) ==
                         std::get<1>(m.first->SubResult[index][SR_index])) &&
                        ...
                        tmp.insert(std::make_tuple(std::get<0>(m.first->SubResult[index][SR_index]),
                                                   std::get<1>(m.first->SubResult[index][SR_index]),
                                                   std::get<2>(m.first->SubResult[index][SR_index])));
                        ex_delay--;
                    }
            ...
            if (ex_delay == 0) {
                std::cout << "RETIMING: Success" << std::endl;
            } else {
                std::cout << "RETIMING: Failed" << std::endl;
            }
        }
    }
    return false;
...
void Router::LocalReTiming(Connection& C, std::set<std::tuple<uint16_t, uint16_t, uint16_t>>& repeater_location) {
    ...
    //     std::cout<<"repeater_location "<<std::get<0>(test)<<" "<<std::get<1>(test)<<"
    //     "<<std::get<2>(test)<<std::endl;
    // }
    ...
    for (auto& n : Result_copy) {
        ...
        if (ptr != repeater_location.end()) {
            ...
            if (next_ptr != C.RouteResult.end()) {
                // std::cout<<"set repeater at "<<std::get<0>(n_copy_y.coord)<<" "<<std::get<1>(n_copy_y.coord)<<"
                // "<<std::get<2>(n_copy_y.coord)<<std::endl;
                C.Result.erase(n);
...
int Router::heuristic(std::vector<Router::coord>& Goals, const Point*& Curr_Node){
    int ret;
    if(Goals.empty()){
        return 0;
    }else{
        ret = std::abs(Goals[0].x-Curr_Node->Loc.x) + std::abs(Goals[0].y-Curr_Node->Loc.y) + std::abs(Goals[0].z-Curr_Node->Loc.z);
        for(auto&it : Goals){
            ret = std::min(ret, std::abs(it.x-Curr_Node->Loc.x) + std::abs(it.y-Curr_Node->Loc.y) + std::abs(it.z-Curr_Node->Loc.z));
        }
    }
    return ret;
}